<?php
  ...
  public function __construct (){ 
    ...
    /*------- CONSOLIDADO PARTIDAS POR REGIONAL (2020) -------*/
/*    public function consolidado_partidas_unidad($dep_id,$tp_id){
      $tabla='';
      ...
      if(count($dep)!=0){
        $consolidado_partidas=$this->mrep_operaciones->partidas_unidad_regional($dep_id,$tp_id);
          $tabla .='
          <style>
            table{font-size: 9px;
              width: 100%;
        ...
        $tabla.='
          <table table border="1" cellpadding="0" cellspacing="0" class="tabla" style="width:70%;">
            <thead>
            <tr style="height:50px;">
              <th align=center colspan=7><b>CONSOLIDADO REGIONAL DE PARTIDA POR UNIDAD/ESTABLECIMIENTO<br>'.$titulo.' - '.$this->gestion.'</b></th>
            </tr>
            <tr style="height:30px;">
              <th style="width:5%; height:30px; background-color: #1c7368; color: #FFFFFF">REGIONAL</th>
              <th style="width:5%; background-color: #1c7368; color: #FFFFFF">TIPO DE ADMINISTRACI&Oacute;N</th>
              <th style="width:5%; background-color: #1c7368; color: #FFFFFF">APERTURA PROGRAMATICA</th>
              <th style="width:15%; background-color: #1c7368; color: #FFFFFF">UNIDAD / ESTABLECIMIENTO / PROYECTO DE INVERSI&Oacute;N</th>
              <th style="width:5%; background-color: #1c7368; color: #FFFFFF">C&Oacute;DIGO</this>
              <th style="width:5%; background-color: #1c7368; color: #FFFFFF">PARTIDA</th>
              <th style="width:5%; background-color: #1c7368; color: #FFFFFF">MONTO PROGRAMADO</th>
            </tr>
            </thead>
            <tbody>';
            $suma=0;
            foreach ($consolidado_partidas as $row){ 
              $tabla.='
              <tr style="height:30px;">
                ...
                <td>';
                  if($row['tp_id']==1){
                    $tabla.=mb_convert_encoding(strtoupper($row['proy_nombre']), 'cp1252', 'UTF-8');
                  }
                  else{
                    $tabla.=mb_convert_encoding(strtoupper($row['tipo'].' '.$row['act_descripcion'].' '.$row['abrev']), 'cp1252', 'UTF-8');
                  }
                $tabla.='
                </td>
                <td>'.$row['par_codigo'].'</td>
                <td>'.mb_convert_encoding(strtoupper($row['par_nombre']), 'cp1252', 'UTF-8').'</td>
                <td align=right bgcolor=#e6e4e4><b>'.round($row['monto'],2).'</b></td>
              </tr>';
              $suma=$suma+$row['monto'];
            }
          $tabla.=
            '</tbody>
              <tr style="height:30px;">
                <td colspan=6><b>TOTAL PROGRAMADO: </b></td>
                <td align=right bgcolor=#e6e4e4><b>'.round($suma,2).'</b></td>
              </tr>
          </table>';

          date_default_timezone_set('America/Lima');
          header('Content-type: application/vnd.ms-excel');
          header("Content-Disposition: attachment; filename=Consolidado_Partidas_Regional.xls"); //Indica el nombre del archivo resultante
          header("Pragma: no-cache");
          header("Expires: 0");
          echo "";
          echo $tabla;
    ...
    /*------- CONSOLIDADO NACIONAL POA PARTIDAS POR UNIDAD -------*/
/*    public function consolidado_nacional_partidas_unidad($tp_id){
      $consolidado=$this->mrep_operaciones->partidas_consolidado_nacional($tp_id);
      $tabla='';
      $titulo=mb_convert_encoding('PROYECTOS DE INVERSI&Oacute;N', 'cp1252', 'UTF-8');
      if($tp_id==4){
        $titulo=mb_convert_encoding('GASTO CORRIENTE', 'cp1252', 'UTF-8');
      ...
      $tabla.='
        <table table border="1" cellpadding="0" cellspacing="0" class="tabla" style="width:40%;">
          <thead>
          <tr style="height:50px;">
            <th align=center colspan=4><b>CONSOLIDADO NACIONAL POR PARTIDA <br>'.$titulo.' - '.$this->gestion.'</b></th>
          </tr>
          <tr style="height:30px;">
            <th style="width:1%; background-color: #1c7368; color: #FFFFFF">#</th>
            <th style="width:5%; background-color: #1c7368; color: #FFFFFF">C&Oacute;DIGO</this>
        ...
        $tabla.=
          '</tbody>
            <tr style="height:30px;">
              <td colspan=3><b>TOTAL PROGRAMADO </b></td>
              <td align=right bgcolor=#e6e4e4><b>'.round($suma,2).'</b></td>
            </tr>
        </table>';
        ...
        date_default_timezone_set('America/Lima');
        header('Content-type: application/vnd.ms-excel');
        header("Content-Disposition: attachment; filename=Consolidado_Nacional_Partidas.xls"); //Indica el nombre del archivo resultante
        header("Pragma: no-cache");
        header("Expires: 0");
        echo "";
        echo "".$tabla."";
    ...
    /*------ LISTA DE OPERACIONES POR REGIONAL (2019-2020) -------*/
/*    public function operaciones_regional_nacional($dep_id,$tp_id){
      $dep=$this->model_proyecto->get_departamento($dep_id);
      $departamento = strtoupper($dep[0]['dep_departamento']);
      if(count($dep)!=0){
        if($this->gestion!=2020){
          $tabla=$this->lista_operaciones_por_regional_distrital($dep_id,1,$tp_id); // Regional Operaciones 2019
        }
        else{
          $operaciones=$this->mrep_operaciones->operaciones_por_regionales($dep_id);
          $dep=$this->model_proyecto->get_departamento($dep_id);
          $titulo='<b>REGIONAL : '.mb_convert_encoding($dep[0]['dep_departamento'], 'cp1252', 'UTF-8').' - '.$this->gestion.'</b>';
          $tabla=$this->lista_operaciones_regional_distrital($operaciones,$titulo); // Regional Operaciones Regional 2020
        }
        
        date_default_timezone_set('America/Lima');
        $fecha = date("d-m-Y H:i:s");
        header('Content-type: application/vnd.ms-excel');
        header("Content-Disposition: attachment; filename=CONSOLIDADO_OPERACIONES_REGIONAL_".$departamento."_$fecha.xls"); //Indica el nombre del archivo resultante
        header("Pragma: no-cache");
        header("Expires: 0");
        echo $tabla;
      }
    ...
    /*--- LISTA DE OPERACIONES NACIONAL POR TIPO DE OPERACION (2020) ---*/
/*    public function operaciones_nacional($tp_id){
      $tabla=$this->lista_operaciones_nacional($tp_id);

      date_default_timezone_set('America/Lima');
      $fecha = date("d-m-Y H:i:s");
      header('Content-type: application/vnd.ms-excel');
      header("Content-Disposition: attachment; filename=CONSOLIDADO_ACTIVIDADES_".$this->gestion."_$fecha.xls"); //Indica el nombre del archivo resultante
      header("Pragma: no-cache");
      header("Expires: 0");
      echo $tabla;
    }*/
