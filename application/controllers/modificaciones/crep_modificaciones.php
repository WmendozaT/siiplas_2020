...
class Crep_modificaciones extends CI_Controller {  
    ...
    /*--------- Reporte Consolidado Modificaciones 2020 -----------*/
    public function reporte_consolidado_2019(){
      ?>
      <style>
      table{font-size: 9px;
            width: 100%;
     ...
     $tabla='';
      $regionales=$this->model_proyecto->list_departamentos();
      $meses=$this->model_modificacion->list_meses();
      ...
      $tabla.='<table class="table table-bordered" align=center>
        <tr bgcolor="#e6e6e6">
          <th></th>
          ...
          if($row['dep_estado']==1){
            ...
            $tabla.='<tr>';
              $tabla.='
                ...
                $sum_ope=0;$sum_req=0;$sum_total=0;
                for ($i=1; $i <=12 ; $i++) { 
                  $o[$i]=$o[$i]+$this->nro_operaciones($row['dep_id'],$i);
                  $r[$i]=$r[$i]+$this->nro_requerimientos($row['dep_id'],$i);
                  $t[$i]=$t[$i]+($this->nro_operaciones($row['dep_id'],$i)+$this->nro_requerimientos($row['dep_id'],$i));

                  $sum_ope=$sum_ope+$this->nro_operaciones($row['dep_id'],$i);
                  $sum_req=$sum_req+$this->nro_requerimientos($row['dep_id'],$i);
                  $sum_total=$sum_total+($this->nro_operaciones($row['dep_id'],$i)+$this->nro_requerimientos($row['dep_id'],$i));

                  $tabla.='
                  <td>'.$this->nro_operaciones($row['dep_id'],$i).'</td>
                  <td>'.$this->nro_requerimientos($row['dep_id'],$i).'</td>
                  <td bgcolor="#cde0c4">'.($this->nro_operaciones($row['dep_id'],$i)+$this->nro_requerimientos($row['dep_id'],$i)).'</td>';
                }

            $tabla.='
              <td bgcolor="#c8eff5">'.$sum_ope.'</td>
              <td bgcolor="#c8eff5">'.$sum_req.'</td>
              <td bgcolor="#c8eff5">'.$sum_total.'</td>
            </tr>';

            $tsum_ope=$tsum_ope+$sum_ope;
            $tsum_req=$tsum_req+$sum_req;
            $tsum_total=$tsum_total+$sum_total;
        ...
        <tr bgcolor="#f5f5f5">
          <td>TOTAL</td>';
            for ($i=1; $i <=12 ; $i++) { 
              ...
              '<td>'.$o[$i].'</td>
              <td>'.$r[$i].'</td>
              <td bgcolor="#cde0c4">'.$t[$i].'</td>';
            }
          $tabla.='
          <td bgcolor="#c8eff5">'.$tsum_ope.'</td>
          <td bgcolor="#c8eff5">'.$tsum_req.'</td>
          <td bgcolor="#c8eff5">'.$tsum_total.'</td>
        </tr>
      </table>';
