...
class User extends CI_Controller{
    ...
    public function dashboard_index(){
        if($this->session->userdata('fun_id')!=null & $this->session->userdata('fun_estado')!=3){
            ...
            if($this->gestion>2020){
                ...
                $nro_poa=count($this->model_seguimientopoa->get_seguimiento_poa_mes_distrital($this->dist,$this->verif_mes[1],$this->gestion));
                if($nro_poa!=0){
                    $data['seguimiento_poa']=$this->mensaje_ejecucion_operaciones_mes($this->dist,$nro_poa);
                }
            }
            else{
    ...
    /*---- NRO DE OPERACIONES A SER EJECUTADAS EN EL MES ----*/
    public function nro_operaciones_mes($dist_id){
        ...
        foreach ($operaciones as $row) {
            ...
            if(count($ejec)!=0){
                if(round($row['pg_fis'],2)>$ejec[0]['pejec_fis']){
                    $nro++;
                }
            }
            else{
    ...
    public function mensaje_ejecucion_operaciones_mes($dist_id,$nro){
        ...
        $tabla.='
            <div class="alert alert-success" role="alert">
                ...
                <hr>
                <p class="mb-0"><a href="#" data-toggle="modal" data-target="#modal_ope_mes" id="'.$dist_id.'" class="ope_mes" title="CAMBIAR GESTI&Oacute;N">Ver Operaciones Programadas</a></p>
            </div>';
    ...
    function validate_credentials(){
        ...
        if(isset($_POST['user_name']) && isset($_POST['password']) && isset($_POST['dat_captcha'])){
            if(preg_match('/^[a-zA-Z0-9_.\/]*$/i', $this->input->post('user_name')) && preg_match('/^[a-zA-Z0-9_.\/]*$/i', $this->input->post('password')) && preg_match('/^[A-Z0-9\/]*$/i', $this->input->post('dat_captcha'))){
